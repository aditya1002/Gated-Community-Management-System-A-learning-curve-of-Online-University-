...
export default function Balancesheetyear(){
    const[fromdate,setnewdate]=useState('');
    const [Incomeyear,setIncomeyear] = useState([]);
    const [Expenditureyear,setExpenditureyear] = useState([]);
    ...
    const Getoutput = () =>{
        var error
        Axios.get(`http://localhost:3001/Balancesheetyear/${fromdate}`).then((response)=>{
            setdatesdata(response.data);
            
        });
        Axios.get(`http://localhost:3001/getIncomeyeartotal/${fromdate}`).then((response)=>{
            setIncometotal(response.data);
            
        });
        Axios.get(`http://localhost:3001/getExpenditureyeartotal/${fromdate}`).then((response)=>{
            setExpendituretotal(response.data);
            
        });
        Axios.get(`http://localhost:3001/getyearIncome/${fromdate}`).then((response)=>{
            setIncomeyear(response.data);
            
        });
        Axios.get(`http://localhost:3001/getyearExpenditure/${fromdate}`).then((response)=>{
            setExpenditureyear(response.data);
    ...
    return (
        <div>
            <Adminnav />
            <Jumbotron>
            <h1> <u>Yearly Balance Sheet</u></h1>
            <Form>
